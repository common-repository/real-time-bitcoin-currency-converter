...
class BC_Controller
{
	...
	const DEFAULT_CURRENCY = BC_ExchangeRates::USD;
	 ...
	 * @return mixed[]
	 */
	public function modify_user_agent(array $http_request_args){
	...
	protected function loadExchangeRates(){
		...
		$this->exchange = new BC_ExchangeRates($exchange_rates);
	}
	...
	public function convert_bitcoin(array $atts, string $content, string $tag)
	{
		...
		$decimals     = isset($atts['decimals']) ? intval($atts['decimals']) : self::DEFAULT_DECIMAL_PLACES;
	 ...
	 * @return array
	 */
	public function register_tiny_mce_buttons( array $buttons )
	{
		array_push( $buttons, 'bitcoin_convert' );
		return $buttons;
	}
	 ...
	 * @return array
	 */
	public function register_tiny_mce_javascript( array $plugin_array )
	{
		$plugin_array['bitcoin_convert'] = plugins_url( '/admin/js/tinymce-plugin.js', __DIR__ );
		return $plugin_array;
	}
	...
	public function register_public_styles() {
		wp_register_style(self::STYLE_FONT_AWESOME, self::FONT_AWESOME_URL, [], self::VERSION, 'all');
	}
	...
	public function enqueue_admin_styles()
	{
		...
		wp_enqueue_style(self::STYLE_ADMIN, plugins_url( '/admin/css/admin-style.css', __DIR__ ), [], self::VERSION);
	}
